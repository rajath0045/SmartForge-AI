...
import { Activity, AlertTriangle, ArrowRight, CalendarClock, Check, CircleGauge, GraduationCap, Search, ShieldCheck, UserCheck, UsersRound } from 'lucide-react';
...
import { ChartTooltip, RadialMeter } from '../components/Analytics';
    ...
    <div className="page-stack">
      ...
      <div className="assets-layout">
        <Panel className="machine-list-panel flush-panel">
          <div className="machine-grid">{shown.map((machine) => <button key={machine.id} className={`machine-card ${selected.id === machine.id ? 'selected' : ''}`} onClick={() => setSelectedId(machine.id)}><div className="machine-card-head"><RadialMeter value={machine.healthScore} label="HLTH" tone={machine.healthScore < 70 ? 'critical' : machine.healthScore < 82 ? 'warning' : 'healthy'} size={48} /><Badge tone={machine.status} dot>{machine.status}</Badge></div><h3>{machine.id}</h3><p>{machine.type}</p><div className="machine-running"><span>{machine.currentOrder ?? 'No active job'}</span><strong>{machine.utilization}% load</strong></div><Progress value={machine.utilization} /><div className="machine-foot"><span>OEE <strong>{machine.oee}%</strong></span><span>Health <strong>{machine.healthScore}</strong></span></div></button>)}</div>
        </Panel>
        <Panel id="machine-detail" tabIndex={-1} className={`machine-detail ${selected.id === bottleneck.id ? 'bottleneck-selected' : ''}`} eyebrow={selected.id === bottleneck.id ? 'BOTTLENECK RESOURCE DETAIL' : 'RESOURCE DETAIL'} title={selected.id} action={<Badge tone={selected.healthScore < 70 ? 'critical' : selected.healthScore < 82 ? 'warning' : 'healthy'}>{selected.healthScore}/100 HEALTH</Badge>}>
          <div className="detail-machine-state"><RadialMeter value={selected.healthScore} label="HEALTH" tone={selected.healthScore < 70 ? 'critical' : selected.healthScore < 82 ? 'warning' : 'healthy'} size={82} /><div><small>CURRENT STATE</small><strong>{selected.status}</strong><span>{selected.currentOrder ? `${selected.currentOrder} in process` : 'Available for dispatch'}</span></div></div>
...
export function WorkforcePage() {
  ...
  const coverage = useMemo(() => skillNames.map((skill) => ({ skill, qualified: operators.filter((operator) => operator.skills[skill] > 0).length, percent: operators.length ? operators.filter((operator) => operator.skills[skill] > 0).length / operators.length * 100 : 0 })), [operators]);
  return (
    <div className="page-stack">
      ...
      <Panel title="Skill coverage by operation" eyebrow="QUALIFIED SHARE OF WORKFORCE" className="skill-coverage-panel"><div className="skill-coverage-visual">{coverage.map((item) => <div key={item.skill}><span><strong>{item.skill}</strong><small>{item.qualified} qualified</small></span><div><i style={{ width: `${item.percent}%` }} /></div><strong>{item.percent.toFixed(0)}%</strong></div>)}</div></Panel>
