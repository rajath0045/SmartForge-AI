...
export function ProfitabilityPage() {
  ...
  const waterfall = [
    { name: 'Revenue', value: 8640, color: '#8ba0aa' }, { name: 'Production', value: -5810, color: '#a86e68' }, { name: 'Overtime', value: -128, color: '#b98270' }, { name: 'Energy', value: -276, color: '#b58f64' }, { name: 'Penalties', value: -164, color: '#c77870' }, { name: 'Rework', value: -84, color: '#9d756b' }, { name: 'Profit', value: 2178, color: '#89aa91' },
  ];
  return (
    <div className="page-stack">
      ...
      <div className="dashboard-grid">
        <InteractiveChartCard className="span-8" title="Expected profit bridge" eyebrow="REVENUE → RISK-ADJUSTED PROFIT" insight="Production cost remains the dominant lever; overtime is justified where it protects penalty-weighted Tier-1 contribution." details={<div className="chart-detail-grid"><span><small>Gross revenue</small><strong>₹86.4L</strong></span><span><small>Total deductions</small><strong>₹64.6L</strong></span><span><small>Expected margin</small><strong>25.2%</strong></span></div>}><div className="chart-lg"><ResponsiveContainer width="100%" height="100%"><BarChart data={waterfall} margin={{ top: 16, left: 4, right: 8 }}><CartesianGrid stroke="#e7ecef" strokeDasharray="3 4" vertical={false} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11 }} /><Tooltip content={<ChartTooltip formatter={(value) => `${value < 0 ? '−' : ''}₹${Math.abs(value).toLocaleString('en-IN')}K`} />} /><Bar dataKey="value" radius={[4, 4, 0, 0]}>{waterfall.map((item) => <Cell key={item.name} fill={item.color} />)}</Bar></BarChart></ResponsiveContainer></div></InteractiveChartCard>
